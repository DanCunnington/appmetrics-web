...
module.exports.start = function(opts) {
	...
	if (typeof(options.server) === 'undefined'){
        ...
        app.use(express.static(directory + '/webdashboard/html'));
		...
		// 				var file = directory + '/html/index.html';
		...
		//options coming in as {app: app, express: express, server: server}
		// serve the files out of ./public as our main files on /monitor
        options.app.use('/monitor', options.express.static(directory + '/webdashboard/html'));
	...
	monitoring.on('express', function (data) {
		io.emit('express', JSON.stringify(data));
	});
